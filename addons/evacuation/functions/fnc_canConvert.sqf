...
if ((_patient getVariable ["ACME_surgicalCasualty", false]) && {
    (([_patient, "Ceftriaxone_IV", false] call ACEFUNC(medical_status,getMedicationCount))
    + ([_patient, "Ceftriaxone", false] call ACEFUNC(medical_status,getMedicationCount))) < 0.9
}) exitWith {false};
