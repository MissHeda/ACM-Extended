...
class SourceTests(unittest.TestCase):
    def test_versions(self):v=re.search(r'version = "([^"]+)"',read_source(ROOT/'config.cpp')).group(1);self.assertIn('version = "'+v+'"',read_source(ROOT/'config.cpp'));self.assertIn('ACME_infusion_version = getText',src('postInit'));assert_release_identity()
    ...
    def test_client_local_accessibility_settings(self):
        ...
        for setting in ('ACME_motion_interpolate','ACME_motion_interpolationTime','ACME_minigameNV_focusBlur'):
            assert_client_only_setting(t,setting)
