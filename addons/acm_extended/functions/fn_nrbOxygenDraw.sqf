...
params ["_patient", "_medic", "_session", "_sequence", ["_hops", 0]];
if (isNull _patient || {isNull _medic}) exitWith {};
if (!local _medic) exitWith {
    if (_hops < 4) then { ["ACME_nrbDraw", [_patient, _medic, _session, _sequence, _hops + 1], _medic] call CBA_fnc_targetEvent; };
};
// [session, highest sequence, last result, patient, last committed time]
private _ledger = +(_medic getVariable ["ACME_nrb_drawLedger", []]);
...
if (_index >= 0 && {_sequence <= _highest}) then {
    ...
    if ((_patient getVariable ["ACME_nrb_session", ""]) == _session
        ...
        _ok = alive _medic && {[_medic] call ACM_breathing_fnc_useOxygenTankReserve};
        _known = true;
        ...
        _medic setVariable ["ACME_nrb_drawLedger", _ledger, true];
    };
};
