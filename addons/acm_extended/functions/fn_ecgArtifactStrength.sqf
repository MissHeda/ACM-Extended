...
private _dp = false;
{if (!isNull (_patient getVariable [format ["ACME_DP_press_%1", _x], objNull])) exitWith {_dp=true;};} forEach ["head","body","leftarm","rightarm","leftleg","rightleg"];
...
if ((_now - (_patient getVariable ["ACM_breathing_BVM_lastBreath", -99])) < 1.2) then {_s = _s max 0.30;};
