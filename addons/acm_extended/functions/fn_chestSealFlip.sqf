...
private _actualSide = [_patient, uiNamespace getVariable ["ACME_CS_Side", "front"]] call ACME_fnc_chestSealActualSide;
_patient setVariable ["ACME_CS_facing", _actualSide, true];
private _newSide = if (_actualSide == "front") then {"back"} else {"front"};
...
// If the body cannot actually be rolled, do not lie to the medic by displaying the opposite surface anyway.
