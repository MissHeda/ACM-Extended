/* B14 acknowledged owner request. Inventory has been reserved, not finally spent.
   _refund lives on supplier, never on patient. Retries preserve id, epoch and catheter generation.
...
private _args = [_patient,_medic,[_patient] call ACME_fnc_clinicalEpoch,_id,_operation,_bodyPart,_doses,_site,_identity];
private _escrow = _medic getVariable ["ACME_medicationEscrow",createHashMap];
_escrow set [_id,[_patient,_args,_refund,0,0]];
