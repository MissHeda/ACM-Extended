...
private _acmeBinding = "NA3:serializeState";
...
// Versioned timestamp metadata. Native JSON values remain backward-compatible.
private _clockRows = [];
{if (!isNil {_unit getVariable _x}) then {_clockRows pushBack [_x, [_unit getVariable _x, true, "cba"] call ACME_fnc_clinicalCodec];};} forEach ["ACM_airway_AirwayObstructionVomit_GracePeriod", "ACM_airway_AirwayChecked_Time", "ACM_breathing_TensionPneumothorax_Time", "ACM_breathing_BVM_lastBreath", "ACM_breathing_BVM_lastBreathOxygen", "ACM_circulation_LozengeItem_InsertTime", "ACM_circulation_ROSC_Time", "ACM_circulation_CardiacArrest_Time", "ACM_circulation_CardiacArrest_DeteriorationTime", "ACM_circulation_ReversibleCardiacArrest_Time", "ACM_circulation_AED_LastShock", "ACM_circulation_CPR_StoppedTime", "ACM_circulation_AmmoniaInhalant_LastUse"];
private _applyTimes = _unit getVariable ["ACM_disability_Tourniquet_ApplyTime", []];
_clockRows pushBack ["ACM_disability_Tourniquet_ApplyTime", ["A", _applyTimes apply {[_x, true, "cba"] call ACME_fnc_clinicalCodec}]];
_state setVariable ["ACME_NativeClocks_v3", _clockRows];

// Convert medications time to offset
private _medications = (_unit getVariable [VAR_MEDICATIONS, []]) apply {+_x};
{
...
_state setVariable ["ACME_Clinical_v3", [_unit] call ACME_fnc_clinicalSnapshot];
