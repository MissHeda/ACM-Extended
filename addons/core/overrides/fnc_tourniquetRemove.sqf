...
private _release = _occludedMedications select {
    (_x select 0) == _partIndex && {!([_patient,_partIndex] call ACME_fnc_aajtOccludes)}
};
_patient setVariable [QACEGVAR(medical,occludedMedications),_occludedMedications - _release,true];
{
    _x params ["_bodyPartN","_medication","_dose","_iv",["_delivery",[]]];
    [QACEGVAR(medical_treatment,medicationLocal),[_patient,_bodyPart,_medication,_dose,_iv,false,_delivery],_patient] call CBA_fnc_targetEvent;
} forEach _release;
