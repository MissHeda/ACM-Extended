...
private _acmeBinding = "NA3:woundsHandlerBase";
if (!local _unit) exitWith {[]};
private _acmeNewWounds = createHashMap;
private _acmeHeadBefore = (GET_BODYPART_DAMAGE(_unit)) param [0, 0];
...
{   // forEach _allDamages
    ...
    for "_i" from 1 to _nWounds do {
        ...
        private _newForPart = _acmeNewWounds getOrDefault [_bodyPart, []];
...
if (_createdWounds) then {
    ...
    if !(_unit getVariable [QGVAR(WasWounded), false]) then {
        _unit setVariable [QGVAR(WasWounded), true, true];
    };

    [_unit, _acmeNewWounds, ((_bodyPartDamage param [0, 0]) - _acmeHeadBefore) max 0] call ACME_fnc_injuryEvent;
