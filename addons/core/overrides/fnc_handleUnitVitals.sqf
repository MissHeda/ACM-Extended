...
if (_elapsed > 7 && (GVAR(NextWarningTime) < CBA_missionTime)) then {
    GVAR(NextWarningTime) = CBA_missionTime + 600;
    WARNING_1("VITALS TIME BETWEEN SYNC IS LONG (%1s)",_elapsed);
};
...
// Native vitals own toxicity timing, independent of Extended circulation switches.
[_unit] call ACME_fnc_medicationToxicityTick;
[_unit] call ACME_fnc_medicationCBRNTick;
...
if (_adjustments isNotEqualTo []) then {
    private _deleted = false;
    {
        ...
        if (_timeInSystem >= _maxTimeInSystem) then {
            ...
            private _effectRatio = ([_administrationType, _timeInSystem, _timeTillMaxEffect, _maxTimeInSystem, _maxEffectTime] call EFUNC(circulation,getMedicationEffect)) * ([_unit, _x] call ACME_fnc_medicationAvailability);
            if (_hrAdjust != 0 && (GET_HEART_RATE(_unit) > 0)) then {
                private _HREffect = [(GET_HEART_RATE(_unit) / ((ACM_TARGETVITALS_HR(_unit) - 20) max 0.001)), ((ACM_TARGETVITALS_HR(_unit) + 20) / GET_HEART_RATE(_unit))] select (_hrAdjust > 0);
...
// B14: medication-specific hemodynamics, shared by bolus and admitted infusion.
private _sedPhys = [_unit] call ACME_fnc_sedationPhysiology;
_hrTargetAdjustment = _hrTargetAdjustment + (_sedPhys select 0) + ([_unit,_deltaT] call ACME_fnc_adenosineTick);
_peripheralResistanceAdjustment = _peripheralResistanceAdjustment + (_sedPhys select 1);
// B18: actual ventilator dyssynchrony supplies only a bounded, slowly accumulated sympathetic stress. The
// respiratory consequences are primary; this adjustment is deliberately too small to be an arrest shortcut.
if (missionNamespace getVariable ["ACME_sys_vent", true]) then {
    _hrTargetAdjustment = _hrTargetAdjustment + (_unit getVariable ["ACME_vent_fightHRAdjust", 0]);
    _peripheralResistanceAdjustment = _peripheralResistanceAdjustment + (_unit getVariable ["ACME_vent_fightResistAdjust", 0]);
};
_respirationRateAdjustment = _respirationRateAdjustment + (_sedPhys select 2);
_coSensitivityAdjustment = _coSensitivityAdjustment + (_sedPhys select 3);
