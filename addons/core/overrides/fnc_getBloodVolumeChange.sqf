            ...
            if ((!([_unit, _partIndex] call ACME_fnc_aajtOccludes)) && ((!(HAS_TOURNIQUET_APPLIED_ON(_unit,_partIndex)) || (!_iv && (_partIndex in [2,3]))) && (([(GET_IO_FLOW_X(_unit,_partIndex)),(GET_IV_FLOW_X(_unit,_partIndex,_accessSite))] select _iv) > 0) && {_type != "FBTK" || {_type == "FBTK" && (_bagVolumeRemaining <= _originalVolume && _iv)}})) then {
                ...
                switch (_type) do {
                    ...
                    case "PlasmaLyte": {
                        ...
                        private _cs = _unit getVariable ["ACME_circ_State", createHashMap];
                        private _metAcid = _cs getOrDefault ["metabolicAcidosis", (_cs getOrDefault ["acidosis", 0])];
                        if (_metAcid > 0) then {
                            _metAcid = (_metAcid - ((_bagChange * _fluidPassRatio) * (missionNamespace getVariable ["ACME_plasmaLyte_acidosisPerMl", 0.0008]))) max 0;
                            _cs set ["metabolicAcidosis", _metAcid];
                            private _respAcid = _cs getOrDefault ["respiratoryAcidosis", 0];
                            _cs set ["totalAcidosis", (_metAcid + _respAcid) min 1];
                            _cs set ["acidosis", (_metAcid + _respAcid) min 1];
                            _unit setVariable ["ACME_circ_State", _cs, _syncValues];
                        };
                    };
                    case "Saline": {
                        _salineVolumeChange = _salineVolumeChange + (_admitted / 1000);
