...
// Resolution-independent ACM design canvas. Controls authored for the historical 16:9 layout use a centered
// height-derived canvas instead of raw safeZoneW. At 16:9 this is identical to the old coordinates; at 21:9 and
// 32:9 the UI keeps the same scale/aspect and gains side gutters instead of stretching horizontally. Narrower
// screens fall back to their available width so controls never render off-screen.
#define ACM_UI_CANVAS_W ((safeZoneW) min (safeZoneH * 1.7777777778))
#define ACM_UI_CANVAS_X (safeZoneX + ((safeZoneW - ACM_UI_CANVAS_W) / 2))
#define ACM_UI_CANVAS_H safeZoneH
#define ACM_UI_CANVAS_Y safeZoneY
