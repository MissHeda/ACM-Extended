...
GitHub Actions run **35793952233**, job **106968747504**, independently verified candidate **27baa723201687b4cfce05570173663872c5712d**, parent **d1b4cd550a7f93d7aa9536c731edcc4958387e45**, and complete source tree **a944bf8bc497b10ecb8428bb0bf798e61fec7a94**. The reviewed patch SHA256 is **35dca9a370faf70b6bb576c4a8df753b7af6a4c5f2513cfa6ea08dd8c01f7b84**. No candidate source was changed to satisfy the independent run.

The focused suite passed **791 tests and 400 subtests**, with no failures/errors/skips, and full-project HEMTT returned **0**. Both broad runs reproduced the table above: twelve original failing identities now pass, zero newly failing identities or comparable passing-outcome regressions, no collection/setup errors, and the same four skips. Both broad-suite commands returned **1**; the successful validation workflow does not mean the historical suite is clean.

The independent unchanged-source control produced **22 failed and 26 passed**. All **48 cases passed** on the corrected candidate. Complete-tree comparison restricted publication to **11 added/modified paths**, including only the two named runtime paths; **4,010 other tracked paths were unchanged and no files were deleted**. The audit workflow and transferred payload are excluded from the candidate. The follow-up commit changes only this report to record the independent evidence.

Raw command exit codes, JUnit results, before/after reports, reviewed patch, preservation manifest and candidate identity are retained in artifact **10723331616**, `historical-backlog-batch4-results`, SHA256 **1087742887ac9388c5e54a4056b68fc69886634c8bbf59fc9bb55368faf0a020**. The accompanying evidence package retains all **460 original failure/error identities**, their earlier assessments and the current **216-entry unresolved source-contract index**. No remaining failure is reclassified as harmless solely because its assertion is old.
